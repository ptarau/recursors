...
def clean_quest(x, sent, context):
    x = x.strip()
    ...
    assert x and x[0:3] in ['Q: ', 'A: '], (x, context)
    return x
...
def to_quests(agent, question, context, short_size=4, k=3):
    ...
    Prefix each answer with "A:", but do NOT number them as in "A1:".
    Then suggest a salient follow-up question to your answer, prefixed with "Q:" . 
    """
    prompt = " ".join(p.split())
