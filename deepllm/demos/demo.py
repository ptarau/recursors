...
from truth_raters import *


def test_truth_rater(
    goal=None, prompter=None, truth_file=None, threshold=None, lim=None
):
...
def run_all():
    ...
    test_truth_rater(
        prompter=sci_prompter,
        goal="Teaching computational thinking with Prolog",
        truth_file="computational_thinking",
        threshold=0.50,
        lim=2,
    )
    ...
    test_truth_rater(
        ...
        truth_file="artificial_general_intelligence",
        threshold=0.50,
        lim=2,
    )

    test_rater(
        prompter=causal_prompter, goal="The Fermi paradox", threshold=0.60, lim=2
    )
    ...
    test_rater(
        prompter=recommendation_prompter, goal="The Godfather", threshold=0.20, lim=2
    )
    ...
    test_advisor(
        prompter=conseq_prompter, goal="Disproof the Riemann hypothesis", lim=2
    )
    test_advisor(prompter=conseq_prompter, goal="Proof the Riemann hypothesis", lim=2)

    test_rater(
        prompter=sci_prompter, goal="Low power circuit design", threshold=0.50, lim=2
    )
